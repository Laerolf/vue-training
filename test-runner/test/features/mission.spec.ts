...
describe('features/missions', () => {
    describe('/requirements', () => {
        test(
            'It should be possible to get the requirements of a mission',
            { timeout: timeouts.WAIT_FOR_PLAYWRIGHT },
            async () => {
                // Given
                const missionId = 'example'

                // When
                const response = await supertest(createTestApi())
                    .get(`/api/missions/${missionId}/requirements`)
    ...
    describe('/verify', () => {
        test(
            'It should be possible to verify a mission',
            { timeout: timeouts.WAIT_FOR_PLAYWRIGHT },
            async () => {
                // Given
                const missionId = 'example'
                const expectedPassingTestId =
                    'a30a6eba6312f6b87ea5-73299983c20d138d5a6e'
                const expectedFailingTestId =
                    'a30a6eba6312f6b87ea5-cf9d7bd3d231719b49f2'
                const expectedFailingStepTitle = 'Test the page title'

                // When
                const response = await supertest(createTestApi())
                    .get(`/api/missions/${missionId}/verify`)
                    .set('Accept', 'application/json')
                ...
                const expectedFailingTest = report.tests.find(
                    (test) => test.id === expectedFailingTestId
                )
                ...
                const expectedPassingTest = report.tests.find(
                    (test) => test.id === expectedPassingTestId
                )
